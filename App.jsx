...
const APP_FILE_EXTENSION = ".ckit";
const APP_STORAGE_KEY = "cabinetkitstudio.autosave.v1";
...
function createDefaultProjectState() {
  return {
    ...
    toeKickHeight: 3,
    defaultCabinetWidth: DEFAULT_CABINET_WIDTH,
    defaultCabinetHeight: DEFAULT_CABINET_HEIGHT_IN,
    defaultCabinetDepth: DEFAULT_CABINET_DEPTH_IN,
    roomWidth: 144,
    roomDepth: 144,
    roomHeight: 108,
    activeWalls: [1],
    expandedWalls: [1, 2, 3, 4],
    viewportWall: 1,
    wallOpenings: {
    ...
    wallCabinets: { 1: [DEFAULT_CABINET_WIDTH], 2: [DEFAULT_CABINET_WIDTH], 3: [DEFAULT_CABINET_WIDTH], 4: [DEFAULT_CABINET_WIDTH] },
    wallCabinetTypes: { 1: ["cabinet"], 2: ["cabinet"], 3: ["cabinet"], 4: ["cabinet"] },
    wallCabinetLocks: { 1: [false], 2: [false], 3: [false], 4: [false] },
    wallCabinetPositionLocks: { 1: [false], 2: [false], 3: [false], 4: [false] },
    wallCabinetHeights: { 1: [DEFAULT_CABINET_HEIGHT_IN], 2: [DEFAULT_CABINET_HEIGHT_IN], 3: [DEFAULT_CABINET_HEIGHT_IN], 4: [DEFAULT_CABINET_HEIGHT_IN] },
    wallCabinetDepths: { 1: [DEFAULT_CABINET_DEPTH_IN], 2: [DEFAULT_CABINET_DEPTH_IN], 3: [DEFAULT_CABINET_DEPTH_IN], 4: [DEFAULT_CABINET_DEPTH_IN] },
    wallOffsets: { 1: 0, 2: 0, 3: 0, 4: 0 },
    wallCabinetSections: {
    ...
    wallUpperWidths: { 1: [DEFAULT_CABINET_WIDTH], 2: [DEFAULT_CABINET_WIDTH], 3: [DEFAULT_CABINET_WIDTH], 4: [DEFAULT_CABINET_WIDTH] },
    wallCountertops: { 1: { enabled: false }, 2: { enabled: false }, 3: { enabled: false }, 4: { enabled: false } },
    wallUpperTypes: { 1: ["cabinet"], 2: ["cabinet"], 3: ["cabinet"], 4: ["cabinet"] },
    wallUpperLocks: { 1: [false], 2: [false], 3: [false], 4: [false] },
    wallUpperPositionLocks: { 1: [false], 2: [false], 3: [false], 4: [false] },
    wallUpperHeights: { 1: [DEFAULT_UPPER_HEIGHT_IN], 2: [DEFAULT_UPPER_HEIGHT_IN], 3: [DEFAULT_UPPER_HEIGHT_IN], 4: [DEFAULT_UPPER_HEIGHT_IN] },
    wallUpperDepths: { 1: [DEFAULT_UPPER_DEPTH_IN], 2: [DEFAULT_UPPER_DEPTH_IN], 3: [DEFAULT_UPPER_DEPTH_IN], 4: [DEFAULT_UPPER_DEPTH_IN] },
    wallDepths: { 1: DEFAULT_CABINET_DEPTH_IN, 2: DEFAULT_CABINET_DEPTH_IN, 3: DEFAULT_CABINET_DEPTH_IN, 4: DEFAULT_CABINET_DEPTH_IN },
    showDimensions: false,
  };
}

function sanitizeProjectData(rawState) {
  const defaults = createDefaultProjectState();
  const raw = rawState && typeof rawState === "object" ? rawState : {};
  if (raw.fileType && raw.fileType !== APP_FILE_TYPE) {
    throw new Error("That file is not a CabinetKit Studio kit.");
  }
  const next = { ...defaults, ...raw, appName: APP_NAME, fileType: APP_FILE_TYPE, version: 1 };
  next.projectName = String(raw.projectName || defaults.projectName);
  next.orbit = Number.isFinite(Number(raw.orbit)) ? Number(raw.orbit) : defaults.orbit;
  next.tilt = Number.isFinite(Number(raw.tilt)) ? Number(raw.tilt) : defaults.tilt;
  next.zoom = Number.isFinite(Number(raw.zoom)) ? Number(raw.zoom) : defaults.zoom;
  next.panX = Number.isFinite(Number(raw.panX)) ? Number(raw.panX) : defaults.panX;
  next.panY = Number.isFinite(Number(raw.panY)) ? Number(raw.panY) : defaults.panY;
  next.cameraMode = raw.cameraMode === "parallel" ? "parallel" : "perspective";
  next.toeKickHeight = Number.isFinite(Number(raw.toeKickHeight)) ? Number(raw.toeKickHeight) : defaults.toeKickHeight;
  next.defaultCabinetWidth = Number.isFinite(Number(raw.defaultCabinetWidth)) ? Number(raw.defaultCabinetWidth) : defaults.defaultCabinetWidth;
  next.defaultCabinetHeight = Number.isFinite(Number(raw.defaultCabinetHeight)) ? Number(raw.defaultCabinetHeight) : defaults.defaultCabinetHeight;
  next.defaultCabinetDepth = Number.isFinite(Number(raw.defaultCabinetDepth)) ? Number(raw.defaultCabinetDepth) : defaults.defaultCabinetDepth;
  next.roomWidth = Number.isFinite(Number(raw.roomWidth)) ? Number(raw.roomWidth) : defaults.roomWidth;
  next.roomDepth = Number.isFinite(Number(raw.roomDepth)) ? Number(raw.roomDepth) : defaults.roomDepth;
  next.roomHeight = Number.isFinite(Number(raw.roomHeight)) ? Number(raw.roomHeight) : defaults.roomHeight;
  next.activeWalls = Array.isArray(raw.activeWalls) ? raw.activeWalls.filter((w) => WALLS.includes(Number(w))).map(Number) : defaults.activeWalls;
  if (!next.activeWalls.length) next.activeWalls = defaults.activeWalls;
  next.expandedWalls = Array.isArray(raw.expandedWalls) ? raw.expandedWalls.filter((w) => WALLS.includes(Number(w))).map(Number) : defaults.expandedWalls;
  if (!next.expandedWalls.length) next.expandedWalls = defaults.expandedWalls;
  next.viewportWall = raw.viewportWall === "all" ? "all" : (WALLS.includes(Number(raw.viewportWall)) ? Number(raw.viewportWall) : defaults.viewportWall);
  next.showDimensions = Boolean(raw.showDimensions);

  WALLS.forEach((wall) => {
    const lowerWidths = Array.isArray(raw.wallCabinets?.[wall]) ? raw.wallCabinets[wall] : defaults.wallCabinets[wall];
    const upperWidths = Array.isArray(raw.wallUpperWidths?.[wall]) ? raw.wallUpperWidths[wall] : defaults.wallUpperWidths[wall];
    next.wallCabinets[wall] = normalizeCabinetWidths(lowerWidths.length || 1, lowerWidths, defaults.defaultCabinetWidth);
    next.wallCabinetTypes[wall] = normalizeCabinetTypes(next.wallCabinets[wall].length, raw.wallCabinetTypes?.[wall] || defaults.wallCabinetTypes[wall]);
    next.wallCabinetLocks[wall] = normalizeCabinetLocks(next.wallCabinets[wall].length, raw.wallCabinetLocks?.[wall] || defaults.wallCabinetLocks[wall]);
    next.wallCabinetPositionLocks[wall] = normalizeCabinetLocks(next.wallCabinets[wall].length, raw.wallCabinetPositionLocks?.[wall] || defaults.wallCabinetPositionLocks[wall]);
    next.wallCabinetHeights[wall] = normalizeCabinetHeights(next.wallCabinets[wall].length, raw.wallCabinetHeights?.[wall] || defaults.wallCabinetHeights[wall]);
    next.wallCabinetDepths[wall] = normalizeCabinetDepths(next.wallCabinets[wall].length, raw.wallCabinetDepths?.[wall] || defaults.wallCabinetDepths[wall]);
    next.wallUpperWidths[wall] = normalizeCabinetWidths(upperWidths.length || 1, upperWidths, defaults.defaultCabinetWidth);
    next.wallUpperTypes[wall] = normalizeCabinetTypes(next.wallUpperWidths[wall].length, raw.wallUpperTypes?.[wall] || defaults.wallUpperTypes[wall]);
    next.wallUpperLocks[wall] = normalizeCabinetLocks(next.wallUpperWidths[wall].length, raw.wallUpperLocks?.[wall] || defaults.wallUpperLocks[wall]);
    next.wallUpperPositionLocks[wall] = normalizeCabinetLocks(next.wallUpperWidths[wall].length, raw.wallUpperPositionLocks?.[wall] || defaults.wallUpperPositionLocks[wall]);
    next.wallUpperHeights[wall] = normalizeCabinetHeights(next.wallUpperWidths[wall].length, raw.wallUpperHeights?.[wall] || defaults.wallUpperHeights[wall], DEFAULT_UPPER_HEIGHT_IN);
    next.wallUpperDepths[wall] = normalizeCabinetDepths(next.wallUpperWidths[wall].length, raw.wallUpperDepths?.[wall] || defaults.wallUpperDepths[wall], DEFAULT_UPPER_DEPTH_IN);
    next.wallDepths[wall] = Number.isFinite(Number(raw.wallDepths?.[wall])) ? Number(raw.wallDepths[wall]) : defaults.wallDepths[wall];
    next.wallOffsets[wall] = Number.isFinite(Number(raw.wallOffsets?.[wall])) ? Number(raw.wallOffsets[wall]) : defaults.wallOffsets[wall];
    next.wallCabinetSections[wall] = {
      lowers: raw.wallCabinetSections?.[wall]?.lowers ?? defaults.wallCabinetSections[wall].lowers,
      uppers: raw.wallCabinetSections?.[wall]?.uppers ?? defaults.wallCabinetSections[wall].uppers,
      upperSyncWidth: raw.wallCabinetSections?.[wall]?.upperSyncWidth ?? defaults.wallCabinetSections[wall].upperSyncWidth,
    };
    next.wallCountertops[wall] = { enabled: Boolean(raw.wallCountertops?.[wall]?.enabled ?? defaults.wallCountertops[wall].enabled) };
    next.wallOpenings[wall] = { ...defaults.wallOpenings[wall], ...(raw.wallOpenings?.[wall] || {}) };
  });
...
function Viewport3D(props) {
  const {
    ...
    typesByWall,
    upperWidthsByWall,
    sectionSettingsByWall,
    countertopSettingsByWall,
    cameraMode = "perspective",
  } = props;
  const pxPerInch = 10;
  ...
  const sceneStyle = useMemo(
    () => ({
      transform: `translate(${panX}px, ${panY}px) scale(${zoom}) rotateX(${tilt}deg) rotateY(${orbit}deg)`,
      transformStyle: "preserve-3d",
    }),
    [orbit, tilt, zoom, panX, panY]
  );

  const viewportStyle = useMemo(
    () => ({
      perspective: cameraMode === "parallel" ? "120000px" : "1400px",
    }),
    [cameraMode]
  );
...
export default function BreakroomConfiguratorV1() {
  ...
  const [projectName, setProjectName] = useState("My Kit");
  ...
  const applyProjectState = (rawState) => {
    const data = sanitizeProjectData(rawState);
    ...
    setToeKickHeight(data.toeKickHeight);
    setDefaultCabinetWidth(data.defaultCabinetWidth);
    setDefaultCabinetHeight(data.defaultCabinetHeight);
    setDefaultCabinetDepth(data.defaultCabinetDepth);
    setRoomWidth(data.roomWidth);
    setRoomDepth(data.roomDepth);
    setRoomHeight(data.roomHeight);
    setActiveWalls(data.activeWalls);
    setExpandedWalls(data.expandedWalls);
    setViewportWall(data.viewportWall);
    setWallOpenings(data.wallOpenings);
    setWallCabinets(data.wallCabinets);
    setWallCabinetTypes(data.wallCabinetTypes);
    setWallCabinetLocks(data.wallCabinetLocks);
    setWallCabinetPositionLocks(data.wallCabinetPositionLocks);
    setWallCabinetHeights(data.wallCabinetHeights);
    setWallCabinetDepths(data.wallCabinetDepths);
    setWallOffsets(data.wallOffsets);
    setWallCabinetSections(data.wallCabinetSections);
    setWallUpperWidths(data.wallUpperWidths);
    setWallCountertops(data.wallCountertops);
    setWallUpperTypes(data.wallUpperTypes);
    setWallUpperLocks(data.wallUpperLocks);
    setWallUpperPositionLocks(data.wallUpperPositionLocks);
    setWallUpperHeights(data.wallUpperHeights);
    setWallUpperDepths(data.wallUpperDepths);
    setWallDepths(data.wallDepths);
    setShowDimensions(data.showDimensions);
  ...
  const handleSaveKit = () => {
    const payload = sanitizeProjectData(buildProjectState());
    const blob = new Blob([JSON.stringify(payload, null, 2)], { type: "application/json" });
    const url = URL.createObjectURL(blob);
    const link = document.createElement("a");
    link.href = url;
    link.download = `${slugifyProjectName(projectName)}${APP_FILE_EXTENSION}`;
  ...
  const handleLoadProject = (event) => {
    ...
    reader.onload = (loadEvent) => {
      try {
        ...
        alert(error instanceof Error ? error.message : "Could not load that kit file.");
      }
    };
  ...
  const handleNewKit = () => {
    const okay = window.confirm("Start a new kit? Current unsaved edits in this tab will be replaced.");
    if (!okay) return;
    localStorage.removeItem(APP_STORAGE_KEY);
    applyProjectState(createDefaultProjectState());
  };
  ...
  useEffect(() => {
    ...
    try {
      localStorage.setItem(APP_STORAGE_KEY, JSON.stringify(sanitizeProjectData(buildProjectState())));
    } catch (error) {
      console.error("Failed to autosave", error);
    }
  }, [projectName, orbit, tilt, zoom, panX, panY, cameraMode, toeKickHeight, defaultCabinetWidth, defaultCabinetHeight, defaultCabinetDepth, roomWidth, roomDepth, roomHeight, activeWalls, expandedWalls, viewportWall, wallOpenings, wallCabinets, wallCabinetTypes, wallCabinetLocks, wallCabinetPositionLocks, wallCabinetHeights, wallCabinetDepths, wallOffsets, wallCabinetSections, wallUpperWidths, wallCountertops, wallUpperTypes, wallUpperLocks, wallUpperPositionLocks, wallUpperHeights, wallUpperDepths, wallDepths, showDimensions]);
  ...
  return (
    <div className="min-h-screen bg-zinc-100 p-4 md:p-6">
      <input
        ref={fileInputRef}
        type="file"
        accept={`${APP_FILE_EXTENSION},application/json,.json`}
        className="hidden"
      ...
      <div className="mx-auto flex max-w-7xl flex-col gap-4 lg:gap-6">
        <div className="rounded-3xl bg-white p-4 shadow-xl md:p-5">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between" ref={topMenuRef}>
            <div className="flex min-w-0 items-center gap-3">
              <div className="min-w-0">
                <div className="text-xs font-semibold uppercase tracking-[0.24em] text-zinc-500">CabinetKit Studio</div>
                <div className="truncate text-lg font-semibold text-zinc-950">{projectName || "My Kit"}</div>
              </div>
            </div>
            ...
            <div className="flex flex-col gap-3 lg:items-end">
              <div className="flex flex-wrap items-center gap-2">
                <div className="relative">
                  <button
                    type="button"
                    className="rounded-xl border px-3 py-2 text-sm font-medium text-zinc-700 transition hover:bg-zinc-50"
                    onClick={() => setActiveTopMenu((prev) => (prev === "file" ? null : "file"))}
                  >
                    File
                  </button>
                  {activeTopMenu === "file" && (
                    <div className="absolute left-0 top-[calc(100%+0.5rem)] z-30 w-56 rounded-2xl border bg-white p-2 shadow-2xl">
                      <button type="button" className="flex w-full rounded-xl px-3 py-2 text-left text-sm text-zinc-700 transition hover:bg-zinc-50" onClick={handleNewKit}>New Kit</button>
                      <button type="button" className="flex w-full rounded-xl px-3 py-2 text-left text-sm text-zinc-700 transition hover:bg-zinc-50" onClick={handleSaveKit}>Save Your Kit</button>
                      <button type="button" className="flex w-full rounded-xl px-3 py-2 text-left text-sm text-zinc-700 transition hover:bg-zinc-50" onClick={() => fileInputRef.current?.click()}>Load Kit</button>
                      <div className="px-3 pb-1 pt-2 text-xs text-zinc-500">Project files save as {APP_FILE_EXTENSION}</div>
                    </div>
                  )}
                </div>
                ...
                <div className="relative">
                  <button
                    type="button"
                    className="rounded-xl border px-3 py-2 text-sm font-medium text-zinc-700 transition hover:bg-zinc-50"
                    onClick={() => setActiveTopMenu((prev) => (prev === "camera" ? null : "camera"))}
                  >
                    Camera
                  </button>
                  {activeTopMenu === "camera" && (
                    <div className="absolute right-0 top-[calc(100%+0.5rem)] z-30 w-72 rounded-2xl border bg-white p-3 shadow-2xl">
                      <div className="text-xs font-semibold uppercase tracking-[0.2em] text-zinc-500">Camera</div>
                      <div className="mt-2 grid grid-cols-2 gap-2">
                        <button type="button" className={`rounded-xl px-3 py-2 text-sm font-medium ${cameraMode === "perspective" ? "bg-zinc-900 text-white" : "border text-zinc-700 hover:bg-zinc-50"}`} onClick={() => { setCameraMode("perspective"); setActiveTopMenu(null); }}>Perspective</button>
                        <button type="button" className={`rounded-xl px-3 py-2 text-sm font-medium ${cameraMode === "parallel" ? "bg-zinc-900 text-white" : "border text-zinc-700 hover:bg-zinc-50"}`} onClick={() => { setCameraMode("parallel"); setActiveTopMenu(null); }}>Parallel</button>
                      </div>
                    </div>
                  )}
                </div>

                <button type="button" className="rounded-xl border px-3 py-2 text-sm font-medium text-zinc-700 transition hover:bg-zinc-50" onClick={handleSaveKit}>Save Your Kit</button>
              </div>

              <div className="flex flex-wrap items-center gap-2 text-sm text-zinc-600">
                <span className="rounded-full bg-zinc-100 px-3 py-1">Autosave on</span>
                <span className="rounded-full bg-zinc-100 px-3 py-1">Camera: {cameraMode}</span>
                <span className="rounded-full bg-zinc-100 px-3 py-1">Viewport: {isAllWallsView ? "All Walls" : `Wall ${viewportWall}`}</span>
              </div>
            </div>
          </div>
        </div>

        <div className={`grid items-start gap-4 lg:gap-6 ${sidebarCollapsed ? "lg:grid-cols-[88px_minmax(0,1fr)]" : "lg:grid-cols-[360px_minmax(0,1fr)]"}`}>
          <div className="rounded-3xl bg-white p-4 shadow-xl md:p-5">
            {sidebarCollapsed ? (
              <div className="flex flex-col items-center gap-4 text-center">
                <div className="rounded-2xl bg-zinc-100 px-3 py-6 [writing-mode:vertical-rl] text-xs font-semibold uppercase tracking-[0.25em] text-zinc-600">Studio Tools</div>
                <button
                  type="button"
                  role="switch"
                  aria-checked={!sidebarCollapsed}
                  aria-label={sidebarCollapsed ? "Show sidebar" : "Hide sidebar"}
                  className={`relative inline-flex h-7 w-12 items-center rounded-full transition ${sidebarCollapsed ? "bg-zinc-300" : "bg-emerald-500"}`}
                  onClick={() => setSidebarCollapsed((prev) => !prev)}
                >
                  <span className={`inline-block h-5 w-5 transform rounded-full bg-white shadow transition ${sidebarCollapsed ? "translate-x-1" : "translate-x-6"}`} />
                </button>
              </div>
            ) : (
              <>
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <div className="flex items-center gap-3">
                      <div className="rounded-xl bg-zinc-100 px-2 py-2 text-[10px] font-semibold uppercase tracking-[0.22em] text-zinc-600">Studio Tools</div>
                      <h1 className="text-2xl font-semibold text-zinc-950">Studio Tools</h1>
                      <button
                        type="button"
                        role="switch"
                        aria-checked={!sidebarCollapsed}
                    ...
                    <p className="mt-2 text-sm text-zinc-600">Keep the room envelope and wall conditions here. File and camera controls now live in the top bar.</p>
                  </div>
                </div>

                <div className="mt-5 space-y-6">
                  <div className="rounded-2xl border bg-zinc-50 p-4 md:p-5">
                    <div>
                      <label className="mb-2 block text-sm font-medium text-zinc-900">Kit Name</label>
                      <input className="w-full rounded-xl border px-3 py-2" type="text" value={projectName} onChange={(e) => setProjectName(e.target.value)} placeholder="My Kit" />
                    </div>

                    <div className="mt-5 text-sm font-semibold text-zinc-900">Room envelope</div>
                    ...
                    <div className="mt-5 grid grid-cols-1 gap-3 sm:grid-cols-2">
                      <div><label className="mb-2 block text-sm font-medium text-zinc-900">Box height</label><input className="w-full rounded-xl border px-3 py-2" type="number" min="20" max="60" step="0.125" value={defaultCabinetHeight} onChange={(e) => setDefaultCabinetHeight(clamp(Number(e.target.value) || DEFAULT_CABINET_HEIGHT_IN, 20, 60))} /></div>
                      <div><label className="mb-2 block text-sm font-medium text-zinc-900">Box depth</label><input className="w-full rounded-xl border px-3 py-2" type="number" min="12" max="36" step="0.125" value={defaultCabinetDepth} onChange={(e) => setDefaultCabinetDepth(clamp(Number(e.target.value) || DEFAULT_CABINET_DEPTH_IN, 12, 36))} /></div>
                    </div>
                    <div className="mt-4">
                      <div className="mb-2 text-sm font-medium text-zinc-900">Cabinet walls</div>
                    ...
                    <div className="mt-4 space-y-4">
                      <div className="flex items-center gap-2 text-left text-xs text-zinc-600">
                        <span>Open a wall to set cabinet sections and wall-specific depth.</span>
                        <span
                          className="inline-flex h-5 w-5 cursor-help items-center justify-center rounded-full border border-zinc-300 bg-white text-[11px] font-semibold text-zinc-600"
                          title="While facing the wall, measure from the right-most wall. Doors and windows reduce the usable cabinet width on that wall. Corner blocker logic also reserves cabinet depth + 4” where active walls intersect."
                        >
                          i
                        </span>
                      </div>
                      {activeWalls.map((wallNumber) => {
                        ...
                        return (
                          <div key={wallNumber} className="rounded-2xl border bg-white p-3 text-left">
                            ...
                            {isExpanded && (
                              <div className="grid grid-cols-2 items-end gap-3 text-left md:grid-cols-4">
                                <div><label className="mb-1 block text-xs font-medium text-zinc-700">Wall depth (in.)</label><input className="w-full rounded-xl border px-3 py-2" type="number" min="12" max="36" step="1" value={wallDepths[wallNumber]} onChange={(e) => updateWallDepth(wallNumber, e.target.value)} /></div>
                                ...
                                <label className="flex items-center justify-start gap-2 text-sm text-zinc-700 md:col-span-4"><input type="checkbox" checked={wallCountertops[wallNumber]?.enabled ?? false} onChange={(e) => setWallCountertops((prev) => ({ ...prev, [wallNumber]: { ...(prev[wallNumber] || {}), enabled: e.target.checked } }))} />Countertop</label>
                              </div>
                            )}
                          </div>
                        );
          ...
          <div className="rounded-3xl bg-white p-5 shadow-xl md:p-6">
            <div className="flex flex-col gap-3 border-b border-zinc-200 pb-4 sm:flex-row sm:items-center sm:justify-between">
              <div>
                <h2 className="text-xl font-semibold text-zinc-950">Viewport</h2>
                <p className="mt-1 text-sm text-zinc-600">Pan, orbit, and inspect the current kit. Cabinet editing still lives in the canvas callout and wall tool panel below.</p>
              </div>
              <div className="flex flex-wrap items-center gap-2">
            ...
            <div ref={sceneRef} onMouseDown={handlePointerDown} onMouseMove={handlePointerMove} onMouseUp={handlePointerUp} onMouseLeave={handlePointerLeave} onWheel={handleWheel} className="relative mt-4 h-[720px] overflow-hidden rounded-2xl border bg-gradient-to-b from-zinc-50 to-zinc-100">
              <Viewport3D
              ...
              />
              <div className="absolute left-4 top-4 max-w-[calc(100%-2rem)] rounded-xl bg-white/90 px-3 py-2 text-sm shadow">zoom {zoom.toFixed(2)}× · pan {panX.toFixed(0)},{panY.toFixed(0)} · camera {cameraMode} · viewport {isAllWallsView ? "all walls" : `wall ${viewportWall}`} · room {roomWidth}×{roomDepth}×{roomHeight} · {movingCabinet !== null ? "moving" : isPanning ? "panning" : isDragging ? "dragging" : "gliding"}</div>
                ...
                <div className="absolute right-4 top-4 w-72 rounded-2xl border bg-white/95 p-4 text-sm shadow-xl backdrop-blur" onMouseDown={(e) => e.stopPropagation()} onClick={(e) => e.stopPropagation()}>
                  <div className="flex items-center justify-between">
                    ...
                    {selectedCabinet !== null && (
                      <button className="rounded-lg border px-2 py-1 text-xs text-zinc-700" onMouseDown={(e) => e.stopPropagation()} onClick={(e) => { e.stopPropagation(); setSelectedCabinet(null); }}>Done</button>
                    )}
                  </div>
                  ...
                  {selectedCabinet === infoCabinetIndex ? (
                    <div className="mt-3 space-y-2">
                      ...
                      <label className="flex items-center gap-2 text-xs text-zinc-700"><input type="checkbox" checked={infoCabinetLocked} onMouseDown={(e) => e.stopPropagation()} onClick={(e) => e.stopPropagation()} onChange={(e) => handleLockWidthChange(e.target.checked)} />Lock width</label>
                      <label className="flex items-center gap-2 text-xs text-zinc-700"><input type="checkbox" checked={infoCabinetPositionLocked} onMouseDown={(e) => e.stopPropagation()} onClick={(e) => e.stopPropagation()} onChange={(e) => handleLockPositionChange(e.target.checked)} />Lock position</label>
                      <button type="button" className="rounded-xl border px-3 py-2 text-xs font-medium text-red-700 disabled:opacity-40" disabled={currentCabinetWidths.length <= 1} onMouseDown={(e) => e.stopPropagation()} onClick={(e) => { e.stopPropagation(); handleDeleteCabinet(); }}>Delete cabinet</button>
            ...
            {!isAllWallsView && (
              <div className="mt-6 rounded-2xl border bg-zinc-50 p-4">
                ...
                <div className="mt-5 rounded-2xl border bg-white p-4">
                  <div className="mb-3 text-sm font-semibold text-zinc-900">Openings · Wall {viewportWall}</div>
                  <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
                    <label className="flex items-center justify-start gap-2 text-sm text-zinc-700"><input type="checkbox" checked={wallOpenings[viewportWall]?.door} onChange={(e) => updateWallOpening(viewportWall, "door", e.target.checked)} />Door</label>
                    <div><label className="mb-1 block text-xs font-medium text-zinc-700">Door distance from right wall (in.)</label><input className="w-full rounded-xl border px-3 py-2" type="number" min="0" step="1" value={wallOpenings[viewportWall]?.doorOffset ?? 0} onChange={(e) => updateWallOpening(viewportWall, "doorOffset", Number(e.target.value) || 0)} disabled={!wallOpenings[viewportWall]?.door} /></div>
                    <div><label className="mb-1 block text-xs font-medium text-zinc-700">Door width trim-to-trim (in.)</label><input className="w-full rounded-xl border px-3 py-2" type="number" min="0" step="1" value={wallOpenings[viewportWall]?.doorWidth ?? 36} onChange={(e) => updateWallOpening(viewportWall, "doorWidth", Number(e.target.value) || 0)} disabled={!wallOpenings[viewportWall]?.door} /></div>
                  </div>
                  <div className="mt-3 grid grid-cols-1 gap-3 md:grid-cols-5">
                    <label className="flex items-center justify-start gap-2 text-sm text-zinc-700"><input type="checkbox" checked={wallOpenings[viewportWall]?.window} onChange={(e) => updateWallOpening(viewportWall, "window", e.target.checked)} />Window</label>
                    <div><label className="mb-1 block text-xs font-medium text-zinc-700">Window distance from right wall (in.)</label><input className="w-full rounded-xl border px-3 py-2" type="number" min="0" step="1" value={wallOpenings[viewportWall]?.windowOffset ?? 0} onChange={(e) => updateWallOpening(viewportWall, "windowOffset", Number(e.target.value) || 0)} disabled={!wallOpenings[viewportWall]?.window} /></div>
                    <div><label className="mb-1 block text-xs font-medium text-zinc-700">Window width (in.)</label><input className="w-full rounded-xl border px-3 py-2" type="number" min="0" step="1" value={wallOpenings[viewportWall]?.windowWidth ?? 48} onChange={(e) => updateWallOpening(viewportWall, "windowWidth", Number(e.target.value) || 0)} disabled={!wallOpenings[viewportWall]?.window} /></div>
                    <div><label className="mb-1 block text-xs font-medium text-zinc-700">Window height (in.)</label><input className="w-full rounded-xl border px-3 py-2" type="number" min="0" step="1" value={wallOpenings[viewportWall]?.windowHeight ?? 36} onChange={(e) => updateWallOpening(viewportWall, "windowHeight", Number(e.target.value) || 0)} disabled={!wallOpenings[viewportWall]?.window} /></div>
                    <div><label className="mb-1 block text-xs font-medium text-zinc-700">Window height from floor (in.)</label><input className="w-full rounded-xl border px-3 py-2" type="number" min="0" step="1" value={wallOpenings[viewportWall]?.windowBottom ?? 36} onChange={(e) => updateWallOpening(viewportWall, "windowBottom", Number(e.target.value) || 0)} disabled={!wallOpenings[viewportWall]?.window} /></div>
                  </div>
                </div>
                <label className="mt-5 flex items-center gap-2 text-sm text-zinc-700"><input type="checkbox" checked={showDimensions} onChange={(e) => setShowDimensions(e.target.checked)} />Show dimensions</label>
                ...
                <div className="mt-5">
                  ...
                  <div className="mt-3 flex flex-wrap gap-2">
                    <button type="button" className="rounded-xl border px-3 py-2 text-sm font-medium text-zinc-700 transition hover:bg-white" onClick={handleCenterRun}>Center Run</button>
                    <button type="button" className="rounded-xl border px-3 py-2 text-sm font-medium text-zinc-700 transition hover:bg-white" onClick={handleAlignLeft}>Align Left</button>
                    <button type="button" className="rounded-xl border px-3 py-2 text-sm font-medium text-zinc-700 transition hover:bg-white" onClick={handleAlignRight}>Align Right</button>
                    <button type="button" className="rounded-xl border px-3 py-2 text-sm font-medium text-zinc-700 transition hover:bg-white" onClick={handleAutoAdjustSizes}>Auto Adjust Sizes</button>
                    <button type="button" className="rounded-xl border px-3 py-2 text-sm font-medium text-zinc-700 transition hover:bg-white" onClick={handleResetDefaultSizes}>Reset Default Sizes</button>
                  </div>
                </div>
              </div>
            )}

            {isAllWallsView && <div className="mt-6 rounded-2xl border bg-zinc-50 p-4 text-sm text-zinc-600">All Walls view is display-only. Select a specific wall tab to edit cabinet count, sizes, offsets, alignment, and cabinet type.</div>}
            <div className="mt-4 text-xs text-zinc-500">Self-tests: {testsPassed ? "passed" : "failed"}</div>
          </div>
        </div>
      </div>
    </div>
